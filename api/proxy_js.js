...
async function redisGet(key) {
  ...
  for (let i = 0; i < 5; i++) {
    if (Array.isArray(val)) {
      // If it's ["[{...}]"] — the inner element is a JSON string, parse it
      if (val.length === 1 && typeof val[0] === "string") {
        try { val = JSON.parse(val[0]); continue; } catch { break; }
      }
      // It's a real array of objects — done
...
// --- Redis SET (pipeline — most reliable Upstash REST format) ---
...
async function scrapeWAAPage(url) {
  try {
    ...
    // --- Cadet name ---
    // Page contains: "Cadet Naomi Miller - Civil Air Patrol - Springfield..."
    const nameMatch =
      html.match(/Cadet\s+([A-Z][a-zA-Z'-]+(?:\s+[A-Z][a-zA-Z'-]+)+)\s*[-–]/i) ||
      html.match(/Cadet\s+([A-Z][a-zA-Z'-]+(?:\s+[A-Z][a-zA-Z'-]+)+)/i);
    const scrapedName = nameMatch ? `Cadet ${nameMatch[1].trim()}` : null;

    // --- Year ---
    // Page contains: "2026 So Far"
    const yearMatch = html.match(/(\d{4})\s+So\s+Far/i);
    const year = yearMatch ? parseInt(yearMatch[1], 10) : new Date().getFullYear();

    // --- Wreaths sold ---
    // Page contains: "0 Wreaths so Far"  or  "12 Wreaths so Far"
  ...
  try {
    ...
    if (req.method === "POST" && action === "add") {
      ...
      const newCadet = {
        ...
        name,                               // manually entered name — always authoritative
        url: waaUrl,
        goal: parseInt(goal, 10) || 0,      // manually entered personal goal
        sold: scraped.sold,
        year: scraped.year,
    ...
    // ---- REFRESH: re-scrape all cadets, preserve name + goal ----
    if (req.method === "POST" && action === "refresh") {
      ...
      const updated = await Promise.all(
        cadets.map(async c => {
          ...
          return {
            ...c,                         // preserve id, name, url, goal
            sold: scraped.sold,           // update live data
            year: scraped.year,
            lastUpdated: scraped.lastUpdated,
            scrapeError: scraped.scrapeError
          };
